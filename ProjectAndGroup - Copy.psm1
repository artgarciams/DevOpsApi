...
function Get-AllFieldsWorkItemType()
{
    ...
    Write-Output "Page|Group|Field Id|Label|Field Type" | Out-File -FilePath $OutputFile

    # loop thru all pages in layout
    foreach ($page in $inheritProc.layout.pages) 
    {
        Write-Output $page.name "|" | Out-File -FilePath $OutputFile -Append -NoNewline

        # loop thru all sections on each page
        foreach ($section in $page.sections) 
        {
            #Write-Output $section.name "|" | Out-File -FilePath $OutputFile -Append -NoNewline

            # loop thru all groups in each section
            foreach ($group in $section.groups) 
            {
                Write-Output  $group.name "|" | Out-File -FilePath $OutputFile -Append -NoNewline
                ...
                foreach ($control in $group.controls) 
                {
                    ...
                    {
                        Write-Output $fld.id  | Out-File -FilePath $OutputFile -Append -NoNewline
                        Write-Output $fld.label  | Out-File -FilePath $OutputFile -Append -NoNewline
                        Write-Output $fld.controlType  | Out-File -FilePath $OutputFile -Append -NoNewline
                ...
                foreach ($currSection in $Curritem.sections) 
                {
                    ...
                    foreach ($newSection in $pgExists.sections) 
                    {
                        ...
                        {                         
                            ...
                            {
                                    # does new group exist ?
                                    # if its one of the default fields we need to loof for them first
                                    $newGrp = $newSection.groups | Where-Object {$_.id -eq $grp.id}                                  
                                  
                                    # does the group already exist ?
                                    if([string]::IsNullOrEmpty($newGrp))
                                    {
                                        
                                        # this is a control on the page not in a group
                                        if($grp.controls[0].controlType -eq "HtmlFieldControl")
                                        {
                                            ...
                                            $addCtl = @{
                                                ...
                                                order = $null
                                                readOnly = "$false"
                                                label =  $grp.controls[0].label
                                                # must encapsulate true false in quotes to register
                                                defaultValue = if($grp.controls[0].controlType -eq "boolean"){"$false"}else {""}
                                                required = if($grp.controls[0].controlType -eq "boolean"){"$true"}else {"$false"}                                                    
                                            }
                                            ...
                                            $fieldURL = $userParams.HTTP_preFix + "://dev.azure.com/" + $userParams.VSTSMasterAcct + "/_apis/work/processes/" + $proc.typeId  + "/workitemtypes/" + $newWKItem.referenceName + "/fields?api-version=6.0-preview.2"
                                            ...
                                            # add group to page
                                            # https://docs.microsoft.com/en-us/rest/api/azure/devops/processes/groups/add?view=azure-devops-rest-7.1
                                            # POST https://dev.azure.com/{organization}/_apis/work/processes/{processId}/workItemTypes/{witRefName}/fields?api-version=7.1-preview.2
                                            ...
                                            $group = Invoke-RestMethod -Uri $groupURL -Method Post -ContentType "application/json" -Headers $authorization -Body $ctlJSON
                                            Write-Host $group

                                            $addCtl = @{
                                                        
                                                id = $grp.controls[0].id.replace("Custom.","")
                                                isContribution = $null
                                                height = $null                                                
                                                label = $grp.controls[0].label
                                                metadata = $null
                                                order = $null
                                                overridden = $null
                                                readOnly = $grp.controls[0].readOnly
                                                visible = $grp.controls[0].visible
                                                watermark = $null                                                                                                
                                            }
                                            $ctlJSON = ConvertTo-Json -InputObject $addCtl
                                    ...
                                    else
                                    {
                                        ...
                                        foreach ($grpCtl in $grp.controls) 
                                        {
                                            ...
                                            $fieldURL = $userParams.HTTP_preFix + "://dev.azure.com/" + $userParams.VSTSMasterAcct + "/_apis/work/processes/" + $proc.typeId  + "/workitemtypes/" + $newWKItem.referenceName + "/fields/" + $field.referenceName + '?$expand=all&api-version=7.1-preview.2'
                                            $witField = Invoke-RestMethod -Uri $fieldURL -Method Get -ContentType "application/json" -Headers $authorization 

                                            Write-Host $witField
                                            if($grpCtl.isContribution -eq $true)
                                            {
                                                $addCtl = @{
                                                    isContribution = $grpCtl.isContribution
                                                    height = $null                                                
                                                    inherited = $grpCtl.inherited
                                                    label = $grpCtl.label
                                                    metadata = $null
                                                    order = $null
                                                    overridden = $null
                                                    readOnly = $grpCtl.readOnly
                                                    visible = $grpCtl.visible
                                                    watermark = $null                                                                                                
                                                    contribution = @{
                                                        contributionId = $grpCtl.contribution.contributionId
                                                        inputs = @{
                                    ...
                                    {   
                                        # group exists update the group
                                        # deployment and development groups inherited and trying to hide
                                        if($grp.inherited -eq $true)
                                        {
                                            $editGrp = @{
                                                id = $newGrp.Id
                                                label = $grp.label
                                        ...
                                        {
                                            # update group to not visible
                                            $editGrp = @{
                                                id = $newGrp.Id
                                                label = $grp.label
                                                visible = if($grp.visible -eq "true"){"$true"}else{"$false"}
                                            }
                                        }
